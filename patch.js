...
const replacement = `    const fetchCatalogue = async () => {
        try {
            // Fetch DB Products
...
const newContent = content.replace(/    const verifyAuth = async \(\) => {/, replacement);

const matchToReplace = `          // Fetch DB Products
          let prodQuery = supabase.from('nutrition_products').select('*');
          const { data: dbProds } = await prodQuery;
          if (dbProds && dbProds.length > 0) {
             const grouped = dbProds.reduce((acc: any, p: any) => {
...
const finalContent = newContent.replace(matchToReplace, '');

fs.writeFileSync('src/app/nutrition/page.tsx', finalContent);
