...
// I am going to replace the messed up content inside `activeTab === 'today'` from line 3176.
// `\n            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">` up to `{/* Tracking Eau & Bilan */}`

const errStart = content.indexOf(`<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">`);
const errEnd = content.indexOf(`{/* Tracking Eau & Bilan */}`, errStart);
...
// There's another `</div>\n            </div>\n\n                \n\n                \n                \n                {/* Tracking Eau & Bilan */}` that needs to be replaced.
const errEndIdx2 = content.indexOf(`            {/* Suggestions Boutique */}`, errStart);
if (errStart !== -1 && errEndIdx2 !== -1) {
    content = content.substring(0, errStart) + rightGrid + "\n" + content.substring(errEndIdx2);
