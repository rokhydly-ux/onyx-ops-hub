...
let text = fs.readFileSync('src/app/solutions/onyx-nutritionafricaine/page.tsx', 'utf8');
let sIdx = text.indexOf('{diagStep !== 7 ? (');
let eIdx = text.lastIndexOf('</main>');

if (sIdx > -1 && eIdx > -1) {
  let newText = text.substring(0, sIdx) + bentoStepsLanding + '\n            </div>\n          </div>\n        </div>\n      )}\n    ' + text.substring(eIdx);
  fs.writeFileSync('src/app/solutions/onyx-nutritionafricaine/page.tsx', newText, 'utf8');
  console.log('Replaced successfully in Landing Page');
} else {
  console.log('Not found in Landing Page');
}
