...
// I need to replace the ENTIRE `activeTab === 'today'` with the exact correct 3 column structure.
...
const todayEndIdx = content.indexOf(`{activeTab === 'week' && (\n          <div className="space-y-12 animate-in fade-in slide-in-from-right-4 w-full">`);
let todayContent = content.substring(todayStartIdx, todayEndIdx);

// I will just rebuild the content for `activeTab === 'today'` from scratch since it's completely messed up.
...
content = content.substring(0, todayStartIdx) + newTodayContent + "\n" + content.substring(todayEndIdx);
