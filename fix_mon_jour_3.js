...
if(redoDiagMatch) {
    redoDiagJSX = redoDiagMatch[0].replace('h-48 md:h-56', 'h-40 md:h-48').replace('2. Le grand widget', '1. Le grand widget');
...
if (topMarker.test(pageCode) && redoDiagJSX) {
    pageCode = pageCode.replace(topMarker, `${redoDiagJSX}\n\n                `);
}
