  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        ...
        console.error("Error fetching products for sliders:", err);
      }
    }
    fetchProducts();
  }, [supabase]);
           ...
           <div className="space-y-8 animate-in fade-in slide-in-from-right-4 w-full max-w-4xl mx-auto">
              ...
              {clientOrders.length === 0 ? (
                  ...
                  <div className="flex flex-col gap-4">
                      {clientOrders.map((order: any) => (
                          <div key={order.id} onClick={() => handleOpenOrderDetails(order)} className="bg-white border border-zinc-200 rounded-[2rem] p-6 shadow-sm hover:shadow-md transition-shadow cursor-pointer flex flex-col sm:flex-row sm:items-center justify-between gap-6 relative overflow-hidden group">
                              ...
                              <div className="flex items-center gap-4 shrink-0 pl-4 sm:pl-0 border-t sm:border-t-0 sm:border-l border-zinc-100 pt-4 sm:pt-0">
                                  <div className="flex flex-col items-start sm:items-end gap-1">
                                      ...
                                      {(!order.status || order.status.toUpperCase() === 'NOUVEAU') ? (
                                      ...
                                      ) : order.status.toUpperCase() === 'EN PREPARATION' ? (
                                      ...
                                      ) : order.status.toUpperCase() === 'EXPEDIE' ? (
                                      ...
                                      ) : order.status.toUpperCase() === 'LIVRE' ? (
                                          ...
                                          <span className="bg-red-100 text-red-700 px-3 py-1 rounded-full text-[10px] font-black uppercase flex items-center gap-1"><X size={12}/> {order.status}</span>
                                      )}
                                  </div>
                              </div>
                          </div>
              ...
              <div className="mt-8">
                  <div className="flex justify-between items-center mb-4 px-4">
                      <h3 className="font-black text-sm uppercase">Mes coups de cœur</h3>
                      <button onClick={() => handleTabChange('shop')} className="text-[#39FF14] text-xs font-bold uppercase">Voir tous</button>
                  </div>
              ...
              {_savedShopProducts.length > 0 && (
                <div className="mt-4 mb-8">
                    <div className="flex justify-between items-center mb-4 px-4">
                        <h3 className="font-black text-sm uppercase">Vus récemment</h3>
                    </div>
                    <div className="flex overflow-x-auto space-x-4 px-4 pb-4 scrollbar-hide">
