          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <button onClick={() => handleTabChange('dashboard')} className="flex items-center gap-2 text-zinc-500 hover:text-black font-black uppercase text-[10px] tracking-widest mb-6"><ChevronLeft size={16}/> Retour à l&apos;accueil</button>
                 ...
                 <div className="flex gap-4 overflow-x-auto pb-4 pt-2 scrollbar-none mb-4 relative z-10">
                     ...
                     <div className="flex flex-col items-center gap-1 cursor-pointer shrink-0" onClick={() => { setStoryMode('media'); setStoryPreviewFile(null); setStoryPreviewUrl(null); setStoryCaption(''); storyInputRef.current?.click(); }}>
                         ...
                         <input type="file" accept="image/*,video/mp4" capture="environment" className="hidden" ref={storyInputRef} onChange={(e) => {
                             ...
                             if (file) {
                                 ...
                                 setStoryCaption("");
                                          ...
                                          <div className="flex flex-col items-center gap-1 cursor-pointer shrink-0" onClick={() => { setStoryMode('text'); setStoryPreviewFile(null); setStoryPreviewUrl('text_placeholder'); setStoryCaption(""); }}>
                         <div className="relative w-16 h-16 rounded-full border-2 border-dashed border-zinc-300 p-0.5 flex items-center justify-center bg-zinc-50 hover:bg-zinc-100 transition-colors">
                             <div className="w-full h-full rounded-full flex items-center justify-center bg-gradient-to-br from-zinc-800 to-black text-white font-black text-xl">Aa</div>
                         </div>
                         <span className="text-xs font-poppins text-center mt-1 truncate w-16 text-zinc-600 font-medium">Texte</span>
                     </div>
                           ...
                           {Array.isArray(communityPosts) && communityPosts.length > 0 ? communityPosts.filter(p => showSavedOnly ? p._bookmarkedByMe : true).filter(p => activeFeedFilter === 'recipes' ? (p.tags?.includes('recette') || p.tags?.includes('menu') || p.content?.toLowerCase().includes('recette') || p.content?.toLowerCase().includes('plat') || p.image_url) : true).map((post, idx) => (
                              <React.Fragment key={post.id || idx}>
                                 ...
                                 {idx > 0 && idx % 4 === 0 && activeChallenge && (
                                     <div id={`mobile-challenge-${idx}`} className="block lg:hidden bg-gradient-to-br from-zinc-900 to-black rounded-[2rem] p-0 shadow-xl relative overflow-hidden group transition-all mb-6 border border-zinc-800">
            ...
            {storyPreviewUrl && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[200] flex items-center justify-center p-4">
                <div className="bg-zinc-900 rounded-[2rem] w-full max-w-md max-h-[90vh] overflow-y-auto overflow-hidden shadow-2xl relative border border-zinc-800 flex flex-col custom-scrollbar">
                   ...
                   <div className="w-full aspect-[9/16] bg-black relative flex items-center justify-center overflow-hidden" id="story-preview-container">
                       {storyMode === 'text' ? (
                           <div className={`w-full h-full p-6 flex flex-col justify-center items-center relative ${TEXT_BACKGROUNDS[storyBgIndex || 0].startsWith("url") ? "" : TEXT_BACKGROUNDS[storyBgIndex || 0]} bg-cover bg-center`} style={{ backgroundImage: TEXT_BACKGROUNDS[storyBgIndex || 0].startsWith("url") ? TEXT_BACKGROUNDS[storyBgIndex || 0] : "none", backgroundSize: "cover", backgroundPosition: "center" }}>
                               <p className="text-center text-white text-3xl font-black break-words max-w-full leading-tight">{storyCaption || "Appuyez en bas pour écrire..."}</p>
                               <div className="absolute bottom-4 right-4 text-white/50 text-xs font-black tracking-widest bg-black/30 px-2 py-1 rounded-md backdrop-blur-sm">NAX</div>
                           </div>
                   ...
                   <div className="p-4 bg-zinc-900 border-t border-zinc-800 flex flex-col gap-3">
                       {storyMode === 'text' && (
                           <div className="flex gap-2 overflow-x-auto scrollbar-none pb-2 items-center">
                               {TEXT_BACKGROUNDS.slice(0, showAllStoryBgs ? TEXT_BACKGROUNDS.length : 5).map((bg, idx) => (
                                   <button key={idx} onClick={() => setStoryBgIndex(idx)} className={`w-10 h-10 rounded-full shrink-0 ${bg.startsWith("url") ? "" : bg} bg-cover border-2 ${storyBgIndex === idx ? 'border-[#39FF14]' : 'border-transparent'}`} style={{ backgroundImage: bg.startsWith("url") ? bg : "none", backgroundSize: "cover", backgroundPosition: "center" }}></button>
                               ))}
                               {!showAllStoryBgs && TEXT_BACKGROUNDS.length > 5 && (
                                   <button onClick={() => setShowAllStoryBgs(true)} className="w-10 h-10 rounded-full bg-zinc-800 flex items-center justify-center text-white hover:bg-zinc-700 transition-colors">
                                       <LayoutGrid size={18} />
                                   </button>
                               )}
                           </div>
                       ...
                       <div className="flex items-center gap-3">
                           <input
                              type="text"
                              placeholder={storyMode === 'text' ? "Écrivez votre story..." : "Ajouter une légende..."}
                              value={storyCaption}
                              onChange={(e) => {
                                  if (storyMode === 'text' && e.target.value.length > 150) return;
                           ...
                           {storyMode === 'text' && (
                               <button
                                  onClick={async () => {
                                      try {
                                          const el = document.getElementById('story-preview-container');
                                          if(!el) return;
                                          const html2canvas = (await import('html2canvas')).default;
                                          const canvas = await html2canvas(el, { backgroundColor: null });
                                          const link = document.createElement('a');
                                          link.download = 'story-nax.png';
                           ...
                           <button
                              ...
                              disabled={isUploadingStory || (storyMode === 'text' && !storyCaption.trim())}
