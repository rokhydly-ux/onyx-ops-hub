...
import { 
  ...
  BarChart3, CreditCard, CalendarClock, PhoneCall, Key, ChevronLeft, ShieldCheck, Mail, Video
} from "lucide-react";

const spaceGrotesk = Space_Grotesk({ subsets: ["latin"], weight: ["300", "500", "700"] });
const inter = Inter({ subsets: ["latin"], weight: ["400", "600", "700"] });

// --- ALIGNEMENT STRICT DES 9 SAAS & PRIX (ADMIN = ACCUEIL) ---
...
export default function AdminBentoTerminal() {
  ...
  async function fetchInitData() {
    ...
    setUser(authUser || { full_name: "Admin Hub", email: "admin@onyx.com", role: "admin", avatar_url: "" });
  ...
  const handleUpdatePassword = async () => {
    if (passwordForm.new !== passwordForm.confirm) return alert("Les mots de passe ne correspondent pas.");
  ...
  const saveClient = async () => {
    ...
    if (newClient.id && newClient.id.trim() !== "") {
      ...
      // FIX ID : crypto.randomUUID() garantit que l'ID n'est jamais null
  ...
  // ================= ACTIVATION PARTENAIRE & MESSAGE PRÉ-REMPLI =================
  const approvePartner = async (partner: any) => {
    ...
    const message = `Félicitations ${partner.full_name} ! Votre compte Partenaire OnyxOps est maintenant ACTIF. ✅\n\nIdentifiant : ${partner.contact}\nMot de passe : ${generatedPwd}\nLien d'accès : https://onyxops.com/login\n\nVous pouvez maintenant acheter vos packs, consulter vos gains dans l'espace dopamine et utiliser votre lien parrain.`;
    
    setPartnersList(partnersList.map(p => p.id === partner.id ? {...p, status: 'Approuvé'} : p));
    
    // Ouverture lien WhatsApp
  ...
  const generateArticleAI = () => {
    ...
    setTimeout(() => {
      setArticleForm({ id: "", title: "Digitalisation 2026 : Pourquoi Dakar explose", category: "Social Selling", pack_focus: "Pack Full", content: "L'intelligence artificielle transforme le commerce..." });
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-white font-black uppercase text-3xl">Terminal Onyx...</div>;
    ...
    <div className={`${inter.className} min-h-screen bg-[#F8F9FA] text-black flex overflow-hidden`}>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        <header className="h-20 bg-white/90 backdrop-blur-md border-b border-zinc-200 px-8 flex justify-between items-center z-10 shadow-sm">
          <h1 className={`${spaceGrotesk.className} text-3xl font-black uppercase tracking-tighter`}>{activeTab === 'overview' ? 'Empire Terminal' : activeTab}</h1>
          <div className="flex items-center gap-6">
            <button onClick={() => window.location.href = '/'} className="flex items-center gap-2 text-xs font-bold text-zinc-500 hover:text-black"><Home size={16}/> Site</button>
            <div onClick={() => setShowProfileModal(true)} className="flex items-center gap-3 cursor-pointer p-1.5 pr-4 bg-zinc-50 rounded-full border border-zinc-200">
              <div className="w-10 h-10 rounded-full bg-zinc-200 border border-zinc-300 flex items-center justify-center font-black text-sm shadow-sm italic">AD</div>
        ...
        <div className="flex-1 overflow-y-auto p-8 flex flex-col">
          {activeTab === 'overview' && (
            <div className="space-y-8 animate-in fade-in">
              <div className="text-center mb-10">
                 <div className="inline-flex items-center gap-2 bg-black text-[#39FF14] px-4 py-1.5 rounded-full text-[10px] font-black uppercase mb-4">Onyx Intelligence V3.5</div>
                 <h2 className={`${spaceGrotesk.className} text-8xl font-black uppercase tracking-tighter italic`}>EMPIRE</h2>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="p-8 rounded-[3rem] border border-zinc-200 bg-black text-white shadow-2xl">
                  <p className="text-[10px] font-black uppercase tracking-widest text-[#39FF14]">CA Mensuel</p>
                  <p className={`${spaceGrotesk.className} text-6xl font-black italic`}>3.850.000 F</p>
                ...
                <div className="bg-white p-8 rounded-[3rem] border border-zinc-200 shadow-sm flex flex-col justify-center text-center">
                  ...
                  <p className={`${spaceGrotesk.className} text-6xl font-black text-black italic`}>{leadsList.length || 142}</p>
                </div>
                <div className="bg-white p-8 rounded-[3rem] border border-zinc-200 shadow-sm flex flex-col justify-center text-center">
                  <p className="text-[10px] font-black uppercase text-zinc-400 mb-2">Articles IA</p>
                  <p className={`${spaceGrotesk.className} text-6xl font-black text-black italic`}>{articlesList.length}</p>
                </div>
              </div>
            </div>
          )}
          ...
          {activeTab === 'users' && (
            <div className="space-y-6 animate-in fade-in">
              <div className="flex justify-between items-center bg-white p-6 rounded-[2.5rem] shadow-sm border border-zinc-200">
                <h3 className={`${spaceGrotesk.className} font-black uppercase text-2xl`}>CRM Hub</h3>
                <button onClick={() => { setNewClient({ id: "", full_name: "", contact: "", saas: "Onyx Vente", role: "prospect", expire: "En essai (J-7)", is_partner: false, avatar_url: "" }); setShowAddClientModal(true); }} className="bg-black text-[#39FF14] px-6 py-4 rounded-xl font-black text-[10px] uppercase flex items-center gap-2 tracking-widest hover:scale-105 transition">
                  <UserPlus size={16}/> Ajouter Contact
                </button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredUsers.map(u => (
                  <div key={u.id} className="bg-white p-8 rounded-[2.5rem] border border-zinc-200 shadow-sm hover:border-black transition flex flex-col group relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <button onClick={() => generatePassword(u)} className="text-[9px] font-black uppercase tracking-widest bg-zinc-100 px-3 py-1.5 rounded-lg hover:bg-black hover:text-white transition">Générer Accès</button>
                    </div>
                    <p className="font-black text-xl mb-1 uppercase italic tracking-tight">{u.full_name}</p>
          ...
          {activeTab === 'partners' && (
            <div className="space-y-6 animate-in fade-in">
              ...
              <div className="grid gap-6">
                {partnersList.length === 0 && <p className="text-zinc-500 font-bold uppercase italic text-sm">Aucune demande reçue.</p>}
                  ...
                  <div key={partner.id} className="bg-white p-8 rounded-[2.5rem] border border-zinc-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-6">
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="bg-zinc-50 p-4 rounded-xl border border-zinc-100">
                           <p className="text-[10px] font-black uppercase text-zinc-400 mb-1">YouTube / Réseaux</p>
                           <p className="text-xs font-bold text-blue-600 truncate">{partner.youtube_link || "Aucun lien fourni"}</p>
                        </div>
                      </div>
                    </div>
                    <div>
                      {partner.status === 'En attente' ? (
                        <button onClick={() => approvePartner(partner)} className="bg-black text-[#39FF14] px-8 py-4 rounded-xl text-xs font-black uppercase hover:scale-105 transition shadow-xl">Activer & Message WA</button>
                      ) : (
                        <span className="text-green-700 bg-green-50 px-6 py-4 rounded-xl font-black uppercase text-xs flex items-center gap-2 border border-green-200"><ShieldCheck size={16}/> Compte Actif</span>
                      )}
                    </div>
                  </div>
                ))}
          ...
          {activeTab === 'marketing' && (
            <div className="space-y-8 animate-in fade-in">
              <div className="flex justify-between items-center bg-white p-6 rounded-[2.5rem] shadow-sm border border-zinc-200">
                 <h3 className={`${spaceGrotesk.className} font-black uppercase text-2xl tracking-tighter`}>Marketing Blog IA</h3>
                 <button onClick={generateArticleAI} disabled={isGenerating} className="bg-black text-[#39FF14] px-6 py-4 rounded-xl font-black text-xs uppercase flex items-center gap-2 hover:scale-105 transition shadow-lg">
                    {isGenerating ? "IA en cours..." : <><Sparkles size={16}/> Génération Auto</>}
                 </button>
              </div>
              <div className="grid gap-6">
                {articlesList.map(article => (
                  <div key={article.id} className="bg-white p-8 rounded-[3rem] border border-zinc-200 shadow-sm flex justify-between items-center group">
                    <div className="flex-1">
                      <div className="flex gap-2 mb-2"><span className="text-[9px] font-black uppercase bg-black text-[#39FF14] px-2 py-1 rounded-md">{article.category}</span></div>
                      ...
                      <p className="text-sm font-medium text-zinc-500 line-clamp-1">{article.content}</p>
                    </div>
                    <button onClick={() => { setArticleForm(article); setShowArticleModal(article); }} className="p-3 bg-zinc-100 rounded-xl hover:bg-black hover:text-white transition opacity-0 group-hover:opacity-100"><Edit3 size={18}/></button>
                  </div>
                ))}
              </div>
            </div>
        ...
        <footer className="mt-8 pt-6 border-t border-zinc-200 flex justify-between items-center px-8 text-[10px] font-black uppercase tracking-widest text-zinc-400 bg-white">
          ...
          <p>Dakar, Senegal - Connexion Sécurisée</p>
        </footer>
      </main>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-md rounded-[3rem] p-10 shadow-2xl relative animate-in zoom-in">
            ...
            <div className="space-y-4">
              ...
              <input type="text" placeholder="Contact WhatsApp" value={newClient.contact} onChange={e => setNewClient({...newClient, contact: e.target.value})} className="w-full p-4 bg-zinc-50 border border-zinc-200 rounded-xl text-sm font-bold outline-none focus:border-black" />
      ...
      {showProfileModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-sm rounded-[3rem] p-10 shadow-2xl relative animate-in zoom-in text-center">
            ...
            <div className="w-20 h-20 rounded-full bg-zinc-100 border-4 border-[#39FF14] mx-auto mb-4 flex items-center justify-center font-black text-2xl italic">AD</div>
            <h2 className={`${spaceGrotesk.className} text-2xl font-black uppercase mb-1`}>{user?.full_name}</h2>
            <p className="text-[10px] font-bold text-zinc-400 mb-8 uppercase tracking-widest">{user?.email}</p>
            <div className="space-y-4">
               ...
               <input type="password" value={passwordForm.confirm} onChange={e => setPasswordForm({...passwordForm, confirm: e.target.value})} placeholder="Confirmer" className="w-full p-4 bg-zinc-50 border border-zinc-200 rounded-xl text-sm font-bold outline-none" />
      ...
      {showArticleModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-2xl rounded-[3rem] p-10 shadow-2xl relative animate-in zoom-in">
            ...
            <button onClick={saveArticle} className="w-full py-4 bg-black text-[#39FF14] font-black text-xs uppercase rounded-xl mt-8 shadow-xl">Publier sur le Blog</button>
          </div>
        </div>
      )}
    </div>
